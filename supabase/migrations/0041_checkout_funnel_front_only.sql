-- =============================================================================
-- 0041_checkout_funnel_front_only.sql — funil do checkout só com o FRONT.
--
-- O funil "de 100% que entrou no checkout" agora conta apenas vendas do
-- produto principal (front) nos buckets comprou/cancelado/não pago — order
-- bumps inflavam o número vs. os checkouts do Meta. Só muda o objeto
-- lost.checkout dentro do central_summary; corpo completo abaixo.
-- =============================================================================
...
returns json language sql stable set search_path to 'public' as $function$
  ...
  lost as (
    select count(*) filter (where is_canceled) as canceled_count,
           ...
           count(*) filter (where is_canceled and role = 'principal') as canceled_front,
           count(*) filter (where is_unpaid and role = 'principal') as unpaid_front,
