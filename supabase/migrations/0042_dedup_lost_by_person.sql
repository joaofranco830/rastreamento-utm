-- =============================================================================
-- 0042_dedup_lost_by_person.sql — dedup de "vendas perdidas" por pessoa.
--
-- Problema: a mesma pessoa costuma gerar vários pix/boletos não pagos ou várias
-- tentativas de compra canceladas (cartão recusado) para o MESMO produto, o que
-- inflava "compras canceladas" e "emitidos e não pagos". Além disso, quem depois
-- comprou o produto (por qualquer meio) não deve contar como perda.
--
-- Solução: nova CTE `pp` (pessoa × produto, chave = contact_hash) que colapsa
-- todas as tentativas em 1 e marca se a pessoa acabou comprando aquele produto:
--   * canceladas/não pagos contam 1 por (pessoa, produto) e só se NÃO comprou;
--   * o mesmo vale para pix não pago.
-- Também adiciona 'delayed' (pagamento pendente/em análise) ao balde "não pago"
-- — o dedup remove quem depois converteu.
--
-- Só muda as CTEs o_scoped (+id, +'delayed') e lost (agora sobre `pp`). Refunds,
-- faturamento, nº de vendas e LTV permanecem idênticos.
-- =============================================================================
...
returns json language sql stable set search_path to 'public' as $function$
  ...
  -- Pessoa × produto: colapsa todas as tentativas da mesma pessoa para o mesmo
  -- produto em 1 linha e marca se ela acabou comprando (por qualquer meio).
  pp as (
    select coalesce(contact_hash, 'ord:' || id::text) as pkey, product_id, role,
           bool_or(gross_value > 0 and not is_canceled)                              as bought,
           bool_or(is_canceled)                                                      as has_canceled,
           bool_or(is_unpaid)                                                        as has_unpaid,
    ...
    from o_scoped
    group by 1, product_id, role
  ),
  -- Vendas perdidas DEDUPLICADAS: 1 por (pessoa, produto) e só quem NÃO comprou.
  lost as (
