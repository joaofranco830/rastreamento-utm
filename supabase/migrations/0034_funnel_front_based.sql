-- =============================================================================
-- 0034_funnel_front_based.sql — conversões do funil referentes ao FRONT.
--
-- Conv. checkout e Conv. funil passam a usar o nº de vendas do PRODUTO
-- PRINCIPAL (front), não o total (que inclui order bumps/upsells e inflava a
-- conversão). Denominadores seguem iguais (checkouts / page views).
--
-- A "Ascensão" (taxa de compra de bump/upsell/downsell sobre o front) é
-- derivada na aplicação a partir de sales_by_role + net_sales_principal, então
-- não precisa de coluna nova aqui.
--
-- ADITIVO: recria só central_summary (mesma assinatura). Nada de schema.
-- =============================================================================
...
returns json language sql stable set search_path to 'public' as $function$
  ...
  select json_build_object(
    ...
    'funnel', json_build_object(
      ...
      -- Conv. checkout e Conv. funil: sobre o FRONT (produto principal).
