-- =============================================================================
-- 0040_refunds_and_lost_sales.sql — reembolso correto + bloco de vendas perdidas.
--
-- MUDANÇAS no central_summary:
--   * "canceled" (cartão não aprovado) SAI dos reembolsos e NÃO conta como
--     compra paga nem como reembolso. Vira "vendas perdidas".
--   * chargeback é somado ao total de reembolsos.
--   * paid_count/gross_revenue passam a excluir canceladas; net_sales fica igual
--     (canceladas antes se anulavam). refund_rate agora só refund+chargeback.
--   * novo objeto `lost`: cancelados, emitidos-não-pagos (billet/pix/expired),
--     pix (total/pago/não pago) e o funil do checkout (entrou → comprou / pix
--     não pago / cancelado / abandono).
-- =============================================================================
...
returns json language sql stable set search_path to 'public' as $function$
  ...
  o_scoped as (
    select o.net_value, o.gross_value, o.refunded_value, coalesce(o.status,'') as status,
           ...
           (coalesce(o.status,'') = any(array['billet_printed','printed_billet','expired','waiting_payment','no_funds','overdue','blocked','pre_order','started'])) as is_unpaid
    from orders o
  ...
  lost as (
    select count(*) filter (where is_canceled) as canceled_count,
           coalesce(sum(gross_value) filter (where is_canceled),0) as canceled_value,
           count(*) filter (where is_unpaid) as unpaid_count,
           coalesce(sum(gross_value) filter (where is_unpaid),0) as unpaid_value,
           count(*) filter (where payment_type = 'PIX') as pix_total,
           count(*) filter (where payment_type = 'PIX' and gross_value > 0 and not is_canceled) as pix_paid,
           count(*) filter (where payment_type = 'PIX' and (is_unpaid or is_canceled)) as pix_unpaid
    from o_scoped
  ),
  byrole as (
  ...
  scoped_visitors as (
    select distinct tp.visitor_id from touchpoints tp, cfg
    where tp.project_id = p_project_id and cardinality(cfg.tags) > 0
  ...
  select json_build_object(
    ...
    'lost', json_build_object(
      ...
      'checkout', json_build_object(
        'entered', eff.checkouts, 'bought', o.paid_count, 'unpaid', lost.unpaid_count, 'canceled', lost.canceled_count,
        'abandoned', greatest(0, eff.checkouts - (o.paid_count + lost.unpaid_count + lost.canceled_count))
      )
    ),
