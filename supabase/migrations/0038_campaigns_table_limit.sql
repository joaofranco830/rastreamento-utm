-- =============================================================================
-- 0038_campaigns_table_limit.sql — limite de linhas na Tela Campanhas.
--
-- Adiciona p_limit (default 20) a campaigns_table e aplica LIMIT após o
-- ORDER BY gasto. Reduz drasticamente o tempo de resposta e o volume
-- renderizado (a conta tem centenas de campanhas/anúncios). Nova assinatura
-- de 7 args; a de 6 é removida.
-- =============================================================================

drop function if exists campaigns_table(bigint, text, text[], text[], date, date);

create function campaigns_table(
  p_project_id bigint, p_level text, p_parent_ids text[], p_product_ids text[], p_from date, p_to date, p_limit int default 20
)
returns table(
...
grant execute on function campaigns_table(bigint, text, text[], text[], date, date, int) to authenticated;
