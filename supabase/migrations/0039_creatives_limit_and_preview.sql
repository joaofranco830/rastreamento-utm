-- =============================================================================
-- 0039_creatives_limit_and_preview.sql — limite + ad representativo nos criativos.
--
-- creatives_consolidated ganha p_limit (default 20) e devolve ad_meta_id
-- (o anúncio de maior gasto com aquele nome), usado para pré-visualizar o
-- criativo no Meta.
-- =============================================================================

drop function if exists creatives_consolidated(bigint, text[], date, date);

create function creatives_consolidated(p_project_id bigint, p_product_ids text[], p_from date, p_to date, p_limit int default 20)
returns table(
  name text, ad_meta_id text,
...
language sql stable set search_path to 'public' as $function$
  ...
  ad_name as (
    select a.id as ad_id, a.meta_id as ad_meta, coalesce(a.name, '(sem nome)') as cname
    from ads a
  ...
  spend_by_ad as (
    ...
    from meta_insights_daily where date between p_from and p_to and project_id = p_project_id group by ad_id
  ),
  rep_ad as (
    select an.cname, (array_agg(an.ad_meta order by coalesce(sp.spend,0) desc))[1] as ad_meta_id
    from ad_name an left join spend_by_ad sp on sp.ad_id = an.ad_id
    group by an.cname
  ),
  insights_by_name as (
    select an.cname,
  ...
  select
    ...
    (select ad_meta_id from rep_ad where rep_ad.cname = coalesce(i.cname, o.cname)) as ad_meta_id,
...
grant execute on function creatives_consolidated(bigint, text[], date, date, int) to authenticated;
