...
- *Trade-off:* **vazar o banco ≠ vazar segredo** (a chave não está no banco) — propriedade que o Vault do Supabase não dá tão diretamente; rápido (decifra fora do dashboard); (−) gestão de 1 segredo a mais na Vercel + procedimento de rotação. *Alternativa avaliada:* **Supabase Vault** (pgsodium) — gerenciado, mas a chave vive no mesmo projeto Postgres e a API é mais áspera → preterido para a v3. (Ref.: Principal §34.6 — *maintain data confidentiality (encryption)*.)
